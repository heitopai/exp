...
# 获取该类数据集下的所有数据文件夹
directory = current_path + '\\deletion_result_LCT\\'  + dataset
...
for file in all_files:
    ...
    patterns = {
        ...
        "deletion_edge": re.compile(r"删除的边为： (\d+) (\d+) (\d+)"),
        "dynamic_time": re.compile(r"基于拟阵交的动态删除有向边的求解时间： ([\d\.]+)"),
        "dynamic": re.compile(r"基于拟阵交的动态删除有向边更新DMST后的权重和： (\d+)"),
        "re_matroid_time": re.compile(r"删除有向边后重新执行基于拟阵交算法的DMST的求解时间： ([\d\.]+)"),
        "re_matroid": re.compile(r"删除有向边后重新执行基于拟阵交算法的DMST的权重和： (\d+)"),
        "re_edmonds": re.compile(r"删除有向边后重新执行Edmonds算法求DMST的权重和： (\d+)"),
        "re_edmonds_time": re.compile(r"删除有向边后重新执行Networkx中基于Edmonds算法的DMST求解时间： ([\d\.]+)"),
    ...
    for line in log_lines:
        if patterns["vertices"].search(line):
            ...
            while i<6:
                ...
                i+=1
        match=patterns["deletion_edge"].search(line)
        if match:
            current_data[f"deletion_edge{j}"] = f"{match.group(1)},{match.group(2)},{match.group(3)}"
            i = 0
            while i < 9:
                line1 = next(log_lines, None)
                if line1=="删除该有向边后没有以0为根的有向生成树":
                    break
                match=patterns[search[i]].search(line1)
                if match:
                    if i == 0:
                        current_data[f"deletion_edge{j}_dynamic_iterations"] = int(match.group(1))
                    elif i == 4:
                        current_data[f"deletion_edge{j}_re_matroid_iterations"] = int(match.group(1))
                    else:
                        current_data[f"deletion_edge{j}_"+search[i]] = float(match.group(1))
                i += 1
            j+=1
    results.append(current_data)
    # print(len(results))
    ...
    df_save["avg_dynamic_iterations"]=df[[f"deletion_edge{i}_dynamic_iterations" for i in range(1,4)]].mean(axis=1).round(3)
    df_save["avg_re_matroid_iterations"]=df[[f"deletion_edge{i}_re_matroid_iterations" for i in range(1,4)]].mean(axis=1).round(3)
    df_save["avg_dynamic_time"]=df[[f"deletion_edge{i}_dynamic_time" for i in range(1,4)]].mean(axis=1).round(3)
    df_save["avg_re_matroid_time"]=df[[f"deletion_edge{i}_re_matroid_time" for i in range(1,4)]].mean(axis=1).round(3)
    df_save["avg_re_edmonds_time"]=df[[f"deletion_edge{i}_re_edmonds_time" for i in range(1,4)]].mean(axis=1).round(3)
