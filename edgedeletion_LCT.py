...
for folder in folders:
    ...
    import sys
    # 将标准输出重定向到文件
    os.makedirs(current_path + '\\deletion_result_LCT\\'  + dataset, exist_ok=True)
    sys.stdout = open(current_path + '\\deletion_result_LCT\\'  + dataset + '\\log' + folder + '.txt', 'w', encoding='utf-8')
    ...
    for file in all_files:
        ...
        for deleted_edge in deleted_edges:
            ...
            if sum_weight>1000000:
                ...
                start=time.time()
                T_list_deleted, AG_adj_deleted, _=DMST(A_list_deleted,G_adj_deleted,num_vertices,len(A_list_deleted),root)
                end=time.time()
